...
pub extern fn basename(__filename: [*c]const u8) [*c]u8; // /nix/store/6jk1d1m5j9d8gjyq79zqlgqqs9j3gcwn-glibc-2.38-44-dev/include/bits/timex.h:81:3: warning: struct demoted to opaque type - has bitfield
...
pub fn proc_info(arg_pid: pid_t, arg_ttynr: [*c]c_int, arg_starttime: [*c]c_ulonglong) callconv(.C) c_int {
    var pid = arg_pid;
    var ttynr = arg_ttynr;
    var starttime = arg_starttime;
    ...
    p = @as([*c]u8, @ptrCast(@alignCast(&buf)));
    n = snprintf(@as([*c]u8, @ptrCast(@alignCast(&path))), @sizeOf([128]u8), "/proc/%d/stat", pid);
    if ((n < @as(c_int, 0)) or (n >= @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([128]u8))))))) return -@as(c_int, 1);
    if ((blk: {
        const tmp = open(@as([*c]u8, @ptrCast(@alignCast(&path))), @as(c_int, 0) | @as(c_int, 131072));
        fd = tmp;
        break :blk tmp;
    }) == -@as(c_int, 1)) {
        warn("failed to open: %s", @as([*c]u8, @ptrCast(@alignCast(&path))));
        return -@as(c_int, 1);
    }
    while ((blk: {
        const tmp = @as(c_int, @bitCast(@as(c_int, @truncate(read(fd, @as(?*anyopaque, @ptrCast(p)), @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(@as([*c]u8, @ptrCast(@alignCast(&buf))) + (@sizeOf([1024]u8) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))))) -% @intFromPtr(p))), @sizeOf(u8)))))))));
        n = tmp;
        break :blk tmp;
    }) != @as(c_int, 0)) {
        if (n == -@as(c_int, 1)) {
            if ((__errno_location().* == @as(c_int, 11)) or (__errno_location().* == @as(c_int, 4))) continue;
            warn("read: %s", @as([*c]u8, @ptrCast(@alignCast(&path))));
            _ = close(fd);
            return -@as(c_int, 1);
        }
        p += @as(usize, @bitCast(@as(isize, @intCast(n))));
        if (p >= (@as([*c]u8, @ptrCast(@alignCast(&buf))) + (@sizeOf([1024]u8) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))))))) break;
    }
    _ = close(fd);
    if ((n != @as(c_int, 0)) or (memchr(@as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&buf))))), @as(c_int, '\x00'), @as(c_ulong, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(p) -% @intFromPtr(@as([*c]u8, @ptrCast(@alignCast(&buf)))))), @sizeOf(u8)) - @as(c_long, @bitCast(@as(c_long, @as(c_int, 1))))))) != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))) {
        warn("NUL in: %s", @as([*c]u8, @ptrCast(@alignCast(&path))));
        return -@as(c_int, 1);
    }
    p.* = '\x00';
    if ((blk: {
        const tmp = strrchr(@as([*c]u8, @ptrCast(@alignCast(&buf))), @as(c_int, ')'));
        p = tmp;
        break :blk tmp;
    }) == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return -@as(c_int, 1);
    n = 2;
    {
        _ = blk: {
            const tmp = strtok_r(p, " ", &saveptr);
            p = tmp;
            break :blk tmp;
        };
        while (p != null) : (_ = blk: {
            const tmp = strtok_r(null, " ", &saveptr);
            p = tmp;
            break :blk tmp;
        }) {
            while (true) {
                switch (blk: {
                    ...
                    @as(c_int, 7) => {
                        ttynr.* = @as(c_int, @bitCast(@as(c_int, @truncate(strtonum(p, @as(c_longlong, @bitCast(@as(c_longlong, -@as(c_int, 2147483647) - @as(c_int, 1)))), @as(c_longlong, @bitCast(@as(c_longlong, @as(c_int, 2147483647)))), &errstr)))));
                        if (errstr != null) return -@as(c_int, 1);
                        break;
                    },
                    @as(c_int, 22) => {
                        __errno_location().* = 0;
                        starttime.* = strtoull(p, &ep, @as(c_int, 10));
                        if ((p == ep) or ((__errno_location().* == @as(c_int, 34)) and (starttime.* == ((@as(c_ulonglong, @bitCast(@as(c_longlong, 9223372036854775807))) *% @as(c_ulonglong, 2)) +% @as(c_ulonglong, 1))))) return -@as(c_int, 1);
                        return 0;
                    },
                    else => {},
                }
...
pub fn timestamp_path(arg_buf: [*c]u8, arg_len: usize) callconv(.C) c_int {
    var buf = arg_buf;
    var len = arg_len;
    var ppid: pid_t = undefined;
    var sid: pid_t = undefined;
    var starttime: c_ulonglong = undefined;
    var n: c_int = undefined;
    var ttynr: c_int = undefined;
    ppid = getppid();
    if ((blk: {
        const tmp = getsid(@as(c_int, 0));
        sid = tmp;
        break :blk tmp;
    }) == -@as(c_int, 1)) return -@as(c_int, 1);
    if (proc_info(ppid, &ttynr, &starttime) == -@as(c_int, 1)) return -@as(c_int, 1);
    n = snprintf(buf, len, "/run/doas/%d-%d-%d-%llu-%d", ppid, sid, ttynr, starttime, getuid());
    if ((n < @as(c_int, 0)) or (n >= @as(c_int, @bitCast(@as(c_uint, @truncate(len)))))) return -@as(c_int, 1);
    return 0;
}
pub export fn timestamp_set(arg_fd: c_int, arg_secs: c_int) c_int {
    var fd = arg_fd;
    var secs = arg_secs;
    var ts: [2]struct_timespec = undefined;
    var timeout: struct_timespec = struct_timespec{
        .tv_sec = @as(__time_t, @bitCast(@as(c_long, secs))),
        .tv_nsec = @as(__syscall_slong_t, @bitCast(@as(c_long, @as(c_int, 0)))),
    };
    if ((clock_gettime(@as(c_int, 7), &ts[@as(c_uint, @intCast(@as(c_int, 0)))]) == -@as(c_int, 1)) or (clock_gettime(@as(c_int, 0), &ts[@as(c_uint, @intCast(@as(c_int, 1)))]) == -@as(c_int, 1))) return -@as(c_int, 1);
    while (true) {
        (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec = (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec + (&timeout).*.tv_sec;
        (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec = (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec + (&timeout).*.tv_nsec;
        if ((&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec >= @as(c_long, 1000000000)) {
            (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec += 1;
            (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec -= @as(c_long, 1000000000);
        }
        if (!false) break;
    }
    while (true) {
        (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec = (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec + (&timeout).*.tv_sec;
        (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec = (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec + (&timeout).*.tv_nsec;
        if ((&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec >= @as(c_long, 1000000000)) {
            (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec += 1;
            (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec -= @as(c_long, 1000000000);
        }
        if (!false) break;
    }
    return futimens(fd, @as([*c]struct_timespec, @ptrCast(@alignCast(&ts))));
}
pub fn timestamp_check(arg_fd: c_int, arg_secs: c_int) callconv(.C) c_int {
    var fd = arg_fd;
    var secs = arg_secs;
    var ts: [2]struct_timespec = undefined;
    var timeout: struct_timespec = struct_timespec{
    ...
    if (fstat(fd, &st) == -@as(c_int, 1)) {
        err(@as(c_int, 1), "fstat");
    }
    if (((st.st_uid != @as(__uid_t, @bitCast(@as(c_int, 0)))) or (st.st_gid != getgid())) or (st.st_mode != @as(__mode_t, @bitCast(@as(c_int, 32768) | @as(c_int, 0))))) {
        errx(@as(c_int, 1), "timestamp uid, gid or mode wrong");
    }
    if (!(((&st.st_atim).*.tv_sec != 0) or ((&st.st_atim).*.tv_nsec != 0)) or !(((&st.st_mtim).*.tv_sec != 0) or ((&st.st_mtim).*.tv_nsec != 0))) return 0;
    if ((clock_gettime(@as(c_int, 7), &ts[@as(c_uint, @intCast(@as(c_int, 0)))]) == -@as(c_int, 1)) or (clock_gettime(@as(c_int, 0), &ts[@as(c_uint, @intCast(@as(c_int, 1)))]) == -@as(c_int, 1))) {
        warn("clock_gettime");
        return 0;
    }
    if (((if ((&st.st_atim).*.tv_sec == (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec) @intFromBool((&st.st_atim).*.tv_nsec < (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec) else @intFromBool((&st.st_atim).*.tv_sec < (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec)) != 0) or ((if ((&st.st_mtim).*.tv_sec == (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec) @intFromBool((&st.st_mtim).*.tv_nsec < (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec) else @intFromBool((&st.st_mtim).*.tv_sec < (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec)) != 0)) return 0;
    while (true) {
        (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec = (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec + (&timeout).*.tv_sec;
        (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec = (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec + (&timeout).*.tv_nsec;
        if ((&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec >= @as(c_long, 1000000000)) {
    ...
    if (((if ((&st.st_atim).*.tv_sec == (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec) @intFromBool((&st.st_atim).*.tv_nsec > (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_nsec) else @intFromBool((&st.st_atim).*.tv_sec > (&ts[@as(c_uint, @intCast(@as(c_int, 0)))]).*.tv_sec)) != 0) or ((if ((&st.st_mtim).*.tv_sec == (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec) @intFromBool((&st.st_mtim).*.tv_nsec > (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_nsec) else @intFromBool((&st.st_mtim).*.tv_sec > (&ts[@as(c_uint, @intCast(@as(c_int, 1)))]).*.tv_sec)) != 0)) {
        warnx("timestamp too far in the future");
        return 0;
    }
    return 1;
}
pub export fn timestamp_open(arg_valid: [*c]c_int, arg_secs: c_int) c_int {
    var valid = arg_valid;
    var secs = arg_secs;
    var ts: [2]struct_timespec = [1]struct_timespec{
        struct_timespec{
            .tv_sec = @as(__time_t, @bitCast(@as(c_long, @as(c_int, 0)))),
            .tv_nsec = @import("std").mem.zeroes(__syscall_slong_t),
        },
    } ++ [1]struct_timespec{@import("std").mem.zeroes(struct_timespec)} ** 1;
    var st: struct_stat = undefined;
    var fd: c_int = undefined;
    var path: [256]u8 = undefined;
    var serrno: c_int = 0;
    valid.* = 0;
    if (stat("/run/doas", &st) == -@as(c_int, 1)) {
        if (__errno_location().* != @as(c_int, 2)) return -@as(c_int, 1);
        if (mkdir("/run/doas", @as(__mode_t, @bitCast(@as(c_int, 448)))) == -@as(c_int, 1)) return -@as(c_int, 1);
    } else if ((st.st_uid != @as(__uid_t, @bitCast(@as(c_int, 0)))) or (st.st_mode != @as(__mode_t, @bitCast(@as(c_int, 16384) | @as(c_int, 448))))) {
        return -@as(c_int, 1);
    }
    if (timestamp_path(@as([*c]u8, @ptrCast(@alignCast(&path))), @sizeOf([256]u8)) == -@as(c_int, 1)) return -@as(c_int, 1);
    fd = open(@as([*c]u8, @ptrCast(@alignCast(&path))), @as(c_int, 0) | @as(c_int, 131072));
    if (fd == -@as(c_int, 1)) {
        var tmp: [256]u8 = undefined;
        var n: c_int = undefined;
        if (__errno_location().* != @as(c_int, 2)) {
            err(@as(c_int, 1), "open: %s", @as([*c]u8, @ptrCast(@alignCast(&path))));
        }
        n = snprintf(@as([*c]u8, @ptrCast(@alignCast(&tmp))), @sizeOf([256]u8), "/run/doas/.tmp-%d", getpid());
        if ((n < @as(c_int, 0)) or (n >= @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([256]u8))))))) return -@as(c_int, 1);
        fd = open(@as([*c]u8, @ptrCast(@alignCast(&tmp))), ((@as(c_int, 0) | @as(c_int, 64)) | @as(c_int, 128)) | @as(c_int, 131072), @as(c_int, 0));
        if (fd == -@as(c_int, 1)) return -@as(c_int, 1);
        if ((futimens(fd, @as([*c]struct_timespec, @ptrCast(@alignCast(&ts)))) == -@as(c_int, 1)) or (rename(@as([*c]u8, @ptrCast(@alignCast(&tmp))), @as([*c]u8, @ptrCast(@alignCast(&path)))) == -@as(c_int, 1))) {
            serrno = __errno_location().*;
            _ = close(fd);
            _ = unlink(@as([*c]u8, @ptrCast(@alignCast(&tmp))));
...
pub export fn timestamp_clear() c_int {
    var path: [256]u8 = undefined;
    if (timestamp_path(@as([*c]u8, @ptrCast(@alignCast(&path))), @sizeOf([256]u8)) == -@as(c_int, 1)) return -@as(c_int, 1);
    if ((unlink(@as([*c]u8, @ptrCast(@alignCast(&path)))) == -@as(c_int, 1)) and (__errno_location().* != @as(c_int, 2))) return -@as(c_int, 1);
    return 0;
}
...
pub const timespeccmp = @compileError("unable to translate C expr: expected ')' instead got 'Identifier'"); // ../include/libopenbsd/sys-time.h:43:9
pub const timespecadd = @compileError("unable to translate C expr: unexpected token 'do'"); // ../include/libopenbsd/sys-time.h:49:9
...
pub const d_fileno = @compileError("unable to translate macro: undefined identifier `d_ino`"); // /nix/store/6jk1d1m5j9d8gjyq79zqlgqqs9j3gcwn-glibc-2.38-44-dev/include/bits/dirent.h:47:9
...
pub const TIMESTAMP_DIR = "/run/doas";
