...
pub var __progname: [*c]const u8 = null;
pub export fn getprogname() [*c]const u8 {
    return __progname;
}
pub export fn setprogname(arg_progname: [*c]const u8) void {
    var progname = arg_progname;
    var last_slash: [*c]const u8 = undefined;
    last_slash = strrchr(progname, @as(c_int, '/'));
    if (last_slash == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        __progname = progname;
    } else {
        __progname = last_slash + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1)))));
    }
}
